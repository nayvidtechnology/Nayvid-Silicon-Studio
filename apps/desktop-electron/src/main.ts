...
export class DesktopBridge {
  ...
  private verifyManifestFiles(root: string, manifest: ProjectManifest): void {
    for (const source of manifest.sources) {
      ...
      if (relative.startsWith('..') || path.isAbsolute(relative)) throw new Error(`Source escapes project root: ${source}`);
      if (!fs.existsSync(absolute) || !fs.statSync(absolute).isFile()) throw new Error(`Project source not found: ${source}`);
    }
    for (const source of manifest.verification?.sources ?? []) {
      const absolute = path.resolve(root, source);
      const relative = path.relative(root, absolute);
      if (relative.startsWith('..') || path.isAbsolute(relative)) throw new Error(`Verification source escapes project root: ${source}`);
      if (!fs.existsSync(absolute) || !fs.statSync(absolute).isFile()) throw new Error(`Verification source not found: ${source}`);
    }
  }
